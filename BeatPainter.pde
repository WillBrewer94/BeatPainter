...
void detectBeat() {
  try {
    if(input == 1) {
      beat.detect(in.mix);
      println(in.mix.level() + .3);
    } else if(input == 2) {
      beat.detect(song.mix);
      println(song.mix.level() + .3);
    }
 ...
 //draw balls, beat line, and waveform
  ...
  for(int i = 0; i < circles.size() - 1; i++) { //balls
    ...
    if(isBeat && (isPlaidDots || isDots)) {
      circles.get(i).setDiameter(40.0);
      ...
      circles.get(i).setDiameter(10.0);
    }
...
void controlEvent(ControlEvent theEvent) throws IndexOutOfBoundsException {
  if(theEvent.isGroup()) {
    if(theEvent.group().getName() == "radioButton") {
      if(theEvent.group().getValue() == 1.0) {
        if(isSong) {
          song.close();
        }
        
        input = 1;
      } else if(theEvent.group().getValue() == 2.0) {
        input = 2;
      }
    }
    ...
    if(theEvent.controller().getName() == "Choose Song") {
      ...
      if(theEvent.controller().getValue() == 0.0) {
        if(isSong) {
          song.close();
        }
      ...
      } else if(theEvent.controller().getValue() == 1.0) {
        if(isSong) {
          song.close();
        }
      ...
      } else if(theEvent.controller().getValue() == 2.0) {
        if(isSong) {
          song.close();
        }
    ...
    if(theEvent.controller().getName() == "Choose Effect") {
      ...
      if(theEvent.controller().getValue() == 1.0) {     
        isPlaid = false;
        isDots = true;
        isPlaidDots = false;       
      }
      
      if(theEvent.controller().getValue() == 2.0) {
        isPlaid = false;
        isDots = false;
        isPlaidDots = true;
      }   
